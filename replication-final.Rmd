...
#test for balance of demographic characteristics between population and those who didn't take second survey
...
#test for balance between outcome questions given demographic effects between complete/ missing groups
...
#test for balance in treatment effect between complete/ missing groups??
...
```{r}
##subset_inference.r
#not quite sure what this does or how it corresponds to the results in the paper.
#it seems to be using randomization?
#it's not clear where all the variables are coming from
...
#not sure what this does. What are ta and tb?	
...
#this does the same thing as above for different ideological subgroups
#not sure where in paper this appears (if at all)
##ideology comparison
final.mat = matrix(nrow = 0, ncol = 8)
...
#not sure what this is or what it corresponds to in the paper (if at all)
##friends comparison
final.mat = matrix(nrow = 0, ncol = 8)
...
#subsetting by income, not sure where/ if this is used in paper
#######income subsets
...
#presumably this is one of the randomization parts
#not clear on what this is trying to demonstrate, although the nonsignficant p-values indicate
#that this is possibly the second randomization test (for missingness)?
...
#not clear what "prime" means or in general how/ why the datasets are split
#this is a good topic to explore for next week
...
#this section and the one following format graphs for the supplemental inforamation portion
#(subsetting by ideology, "friends", and income, and testing facial characteristics)
